    ...
    return (
        <aside className={`bg-secondary border-r border-border-color flex flex-col p-4 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'}`}>
            ...
            <div className="flex items-center mb-8 relative">
                ...
                {!isCollapsed && <h1 className="text-xl font-bold ml-3 text-light whitespace-nowrap">Veo Auto AI</h1>}
            ...
            <div className="border-t border-border-color pt-2">
                ...
                <div
                    className={`${commonItemClasses} ${activeView === AppView.SUPPORT ? activeItemClasses : inactiveItemClasses} ${isCollapsed ? 'justify-center px-2' : 'px-4'}`}
                    onClick={() => setActiveView(AppView.SUPPORT)}
                    title={isCollapsed ? "Hỗ trợ" : undefined}
                >
                    <svg className={`h-6 w-6 transition-colors ${isCollapsed ? '' : 'mr-3'}`} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 5.25h.008v.008H12v-.008Z" /></svg>
                    {!isCollapsed && <span className="font-medium whitespace-nowrap">Hỗ trợ</span>}
                </div>
                 <div
