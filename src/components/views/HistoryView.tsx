...
const HistoryView: React.FC = () => {
    ...
    const handleDownloadVideo = (videoUrl: string | undefined, promptText: string) => {
        if (videoUrl) {
                ...
                return (
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {videos.length === 0 ? <p className="text-dark-text col-span-full text-center py-8">Chưa có video nào.</p> : (
                            videos.map(video => (
                                <div key={video.id} className="bg-secondary p-4 rounded-lg shadow group relative">
                                    <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-2">
                                        {video.status === 'completed' && (video.videoUrl || video.localPath) && (
                                            <button
                                                onClick={() => handleDownloadVideo(video.localPath || video.videoUrl, video.promptText)}
                                                title="Tải về"
                                                className="p-2 rounded-full bg-black/50 text-white hover:bg-accent transition-colors"
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                    ...
                                    <p className="text-gray-500 text-sm truncate mb-2" title={video.promptText}>{video.promptText}</p>
                                    {video.status === 'completed' && (video.videoUrl || video.localPath) ? (
