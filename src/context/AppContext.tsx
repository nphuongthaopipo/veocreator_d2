...
import { fetchProfile } from '../services/authService'; // Import fetchProfile
import { useToast } from './ToastContext';

// Định nghĩa cấu trúc dữ liệu hồ sơ người dùng đầy đủ
...
interface AppContextType {
    ...
    currentUser: ProfileData | null; // Sử dụng ProfileData
    login: (userData: { user: User, token: string }) => Promise<void>; // Chuyển thành hàm async
    logout: () => void;
}
...
export const AppProvider: React.FC<{children: ReactNode}> = ({ children }) => {
    ...
    const login = async (userData: { user: User, token: string }) => {
        try {
            // Gọi API để lấy thông tin hồ sơ (bao gồm gói cước)
            ...
            // Nếu lỗi, vẫn đăng nhập với thông tin cơ bản
    ...
    return (
        <AppContext.Provider value={{
            ...
            automationState, setAutomationState, addAutomationPrompts,
