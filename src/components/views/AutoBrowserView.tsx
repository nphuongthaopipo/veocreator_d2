...
const AutoBrowserView: React.FC = () => {
    ...
    const handleRunSinglePrompt = (promptId: string) => {
        ...
        if (!promptToRun) {
            showToast('Không tìm thấy prompt để chạy.', 'error');
            return;
        }
        ...
        window.electronAPI.startBrowserAutomation({
            prompts: [{ id: promptToRun.id, text: promptToRun.text }],
            authToken: currentUser.token,
            model,
            aspectRatio,
            autoSaveConfig
                             ...
                             <div className="flex justify-between items-center mb-1">
                                ...
                                <div className="flex items-center gap-2">
                                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                                     ...
                                     <button onClick={() => handleRunSinglePrompt(prompt.id)} disabled={isRunning || prompt.status === 'success'} title="Chạy prompt này" className="p-1 hover:bg-green-100 rounded-full disabled:opacity-50">
