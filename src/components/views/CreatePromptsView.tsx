...
const CreatePromptsView: React.FC<CreatePromptsViewProps> = ({ setActiveView }) => {
    const { stories, prompts, addPrompts, deletePrompts, deletePrompt, addAutomationPrompts } = useAppContext();
    ...
    const areAllPromptsSelected = useMemo(() => {
        if (filteredHistoryPrompts.length === 0) return false;
        return selectedPromptIds.length === filteredHistoryPrompts.length;
    }, [selectedPromptIds, filteredHistoryPrompts]);

    const handleSelectAllToggle = () => {
        if (areAllPromptsSelected) {
            setSelectedPromptIds([]);
        } else {
            setSelectedPromptIds(filteredHistoryPrompts.map(p => p.id));
        }
    };
            ...
            <div className="bg-secondary p-4 rounded-lg shadow-md mb-4 flex items-center gap-4 flex-wrap">
                <div className="flex-1 min-w-[200px]">
                    ...
                    <select
                        ...
                        onChange={e => setHistoryFilterStoryId(e.target.value)}
                ...
                <div className="flex items-center gap-2 pt-5">
                     <input
                        ...
                        checked={areAllPromptsSelected}
                        onChange={handleSelectAllToggle}
                        disabled={filteredHistoryPrompts.length === 0}
                    />
                    <label className="text-sm font-medium text-dark-text">Chọn tất cả</label>
                </div>
                    ...
                    filteredHistoryPrompts.map(prompt => (
                        ...
                        >
                            ...
                            <div className="flex-1 cursor-pointer" onClick={() => handleToggleSelectPrompt(prompt.id)}>
                                <p className="text-gray-700 text-sm line-clamp-2">{prompt.prompt}</p>
