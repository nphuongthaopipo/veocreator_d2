...
// =================================================================
// 2. LOGIC TẢI FILE (Cập nhật)
// =================================================================
const downloadFile = async (mainWindow: BrowserWindow, url: string, promptText: string, savePath: string | null, promptIndex: number) => {
    ...
    if (!finalPath) {
        ...
        try {
            ...
            // Tạo tên file duy nhất để tránh ghi đè và bỏ qua kiểm tra file tồn tại
...
ipcMain.on("download-video", async (event, { url, promptText, savePath, promptIndex }) => {
    const mainWindow = BrowserWindow.fromWebContents(event.sender);
    if (!mainWindow) return;
    await downloadFile(mainWindow, url, promptText, savePath, promptIndex);
});


// =================================================================
// 3. LOGIC TỰ ĐỘNG HÓA (Cập nhật)
// =================================================================
ipcMain.on("browser:start-automation", async (event, { prompts, authToken, model, aspectRatio, autoSaveConfig }) => {
    const mainWindow = BrowserWindow.fromWebContents(event.sender);
    ...
    try {
        sendLog(firstPromptId, "Dlcktmc...", "running");
        ...
        sendLog(firstPromptId, `Dlck "${activeCookie.name}". Bdtprj...`, "running");
        ...
        sendLog(firstPromptId, `DtprjID: ${projectId}.`, 'running');

        sendLog(firstPromptId, `Khprj3s...`, 'running');
            ...
            while (promptQueue.length > 0 && !stopAutomationFlag) {
                if (processingPrompts.size < MAX_CONCURRENT_SESSIONS) {
                    ...
                    try {
                        ...
                        sendLog(prompt.id, "Bdgprompt...", "submitting");
                        ...
                        sendLog(prompt.id, "Dgpromt,cph...", "running");
                        ...
                        if (operation?.operation?.name && operation?.sceneId) {
                            ...
                            sendLog(prompt.id, "DnID,CxL...", "processing", null, operation.operation.name, operation.sceneId);
                        } else {
        ...
        const statusManager = async () => {
            while ((promptQueue.length > 0 || processingPrompts.size > 0) && !stopAutomationFlag) {
                ...
                for (const [promptId, data] of promptsToCheck) {
                    ...
                    try {
                        sendLog(promptId, "Sttcheck...", "processing", null, promptData.operationName, promptData.sceneId);
                        ...
                        let statusMessage = "Loading: " + apiStatus.replace("MEDIA_GENERATION_STATUS_", "").toLowerCase();
                        ...
                        if (apiStatus === "MEDIA_GENERATION_STATUS_SUCCESSFUL") {
                            ...
                            if (autoSaveConfig && autoSaveConfig.enabled && autoSaveConfig.path && videoUrl) {
                                sendLog(promptId, `Tự động lưu video vào "${autoSaveConfig.path}"`, "downloading");
                                const originalPrompt = originalPrompts.find(p => p.id === promptId);
                                if(originalPrompt) {
                                    const originalIndex = originalPrompts.indexOf(originalPrompt);
                                    await downloadFile(mainWindow, videoUrl, originalPrompt.text, autoSaveConfig.path, originalIndex);
                                }
                            }
...
// =================================================================
// 4. LOGIC TỰ ĐỘNG CẬP NHẬT
// =================================================================
autoUpdater.autoDownload = false;
autoUpdater.autoInstallOnAppQuit = true;

function sendUpdateMessage(mainWindow: BrowserWindow, message: string, data: any = null) {
    if (mainWindow && !mainWindow.isDestroyed()) {
...
// =================================================================
// 5. HÀM TẠO CỬA SỔ CHÍNH VÀ VÒNG ĐỜI ỨNG DỤNG
// =================================================================
